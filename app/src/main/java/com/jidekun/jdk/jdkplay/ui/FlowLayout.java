...
//            //对子控件进行测量
//            childAt.measure(0, 0);
//            //如果line中没有数据,那就将子控件存入
//            if (line.getViewList().size() == 0) {
//                line.addView(childAt);
//            } else if (line.getLineWidth() + childAt.getMeasuredWidth() + horizontalSpacing > noPaddingWidth) {
//                lineList.add(line);
//                line = new Line();
//                line.addView(childAt);
//            } else {
...
//        int paddingLedft = getPaddingLeft();
...
//            if (i > 0) {
//                paddingTop += line.getLineHeight() + verticalSpacing;
//            }
//
...
//                if (j == 0) {
//                    view.layout(paddingLedft,paddingTop,paddingLedft+view.getMeasuredWidth(),view.getMeasuredHeight());
//                } else {
//                    View preView = viewList.get(j-1);
//                    int left =preView.getMeasuredWidth()+horizontalSpacing;
...
//    //存每一行子View 的容器
//    class Line {
//        //存子view的容器的高,取所有子view中最高的高度
//        private int height;
//        //
//        private int width;
//        //存子View的集合
//        private ArrayList<View> viewLsit;
//
//        //通过构造方法初始化集合
//        public Line() {
//            viewLsit = new ArrayList<>();
//        }
//
//        //向Line中添加子view
//        public void addView(View childView) {
//            //如果不包含就添加进集合
//            if (!viewLsit.contains(childView)) {
//                viewLsit.add(childView);
//                //更新整个Line的宽度
//
//                if (viewLsit.size() == 1) {
//                    width = childView.getMeasuredWidth();   //如果是第一个添加进来的,Line的宽度就的等于添加进来子view的宽度
//                } else {
//                    width += horizontalSpacing + childView.getMeasuredWidth(); //否则用已有的宽度加上子view的宽度在加上一个水平间距
//                }
//                //对比当前Line的高度和新添加进来的子view的高度,2中取大
